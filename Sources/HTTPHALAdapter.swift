...
func parseHALLinks(_ halLinks:[String:Any]) -> [String:[HTTPTransition]] {
...
func parseEmbeddedHALs(_ embeddedHALs:[String:Any]) -> [String:[Representor<HTTPTransition>]] {
  var representors = [String:[Representor<HTTPTransition>]]()

  func parseEmbedded(_ embedded:[String:Any]) -> Representor<HTTPTransition> {
  ...
  for (name, embedded) in embeddedHALs {
    ...
    } else if let embedded = embedded as? [String:Any] {
...
public func deserializeHAL(_ hal:[String:Any]) -> Representor<HTTPTransition> {
  ...
  var links = [String:[HTTPTransition]]()
  if let halLinks = hal.removeValue(forKey: "_links") as? [String:Any] {
    links = parseHALLinks(halLinks)
  }

  var representors = [String:[Representor<HTTPTransition>]]()
  if let embedded = hal.removeValue(forKey: "_embedded") as? [String:Any] {
...
public func serializeHAL(_ representor:Representor<HTTPTransition>) -> [String:Any] {
  ...
  if !representor.transitions.isEmpty {
    ...
    representation["_links"] = links as Any
  }
  ...
  if !representor.representors.isEmpty {
    ...
    representation["_embedded"] = embeddedHALs as Any
  }
