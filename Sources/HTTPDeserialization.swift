...
func jsonDeserializer(_ closure:@escaping (([String:Any]) -> Representor<HTTPTransition>?)) -> ((_ response:HTTPURLResponse, _ body:Data) -> Representor<HTTPTransition>?) {
  return { (response, body) in
    let object: Any? = try? JSONSerialization.jsonObject(with: body, options: JSONSerialization.ReadingOptions(rawValue: 0)) as Any
